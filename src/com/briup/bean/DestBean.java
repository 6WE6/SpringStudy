 ...
 *request:每次请求构建一个对象
 *session:第一次请求构建一个回话对象，在浏览器开启时，再次请求用的时同一个session对象
...
public class DestBean {
	 ...
	 * 1.set方法2.全局变量3.构造器
	 * 
	 * @AutoWired(required=true)表示基于名字或类型一定要找到，找不到就报错，；required=false表示不一定非要找到，找到了就装配，找不到忽略
	 * @Qualifier("")不能用在构造器中，可以用在参数声明中；声明名字，基于名字找到；如果构造器，全局变量，set方法都设置了该注解则set方法生效；如果构造器和全局变量都设置了该注解，则全局变量生效
	 * 
